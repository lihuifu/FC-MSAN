...
from model.FC_MSAN import build_FC_MSAN
from model.DataGenerator import kFoldGenerator
from model.Utils import *

print(128 * '#')
print('Start to train FC_MSAN.')
...
# [train] parameters ('_f' means FC_MSAN)
...
for i in range(fold):
    ...
    opt_f = Instantiation_optim(optimizer_f, learn_rate_f) # optimizer of FC_MSAN
    
    # get i th-fold data
    train_data,train_targets,val_data,val_targets = DataGenerator.getFold(i)
    
    ## build FC_MSAN & train
    featureNet, featureNet_p = build_FC_MSAN(opt_f, channels) # '_p' model is without the softmax layer
    history_fea = featureNet.fit(
      ...
      #  batch_size = batch_size_f,
        ...
        verbose = 2,
        callbacks=[keras.callbacks.ModelCheckpoint(Path['Save']+'FC_MSAN_Best_'+str(i)+'.h5', 
                                                   monitor='val_acc', 
                                                   verbose=0, 
                                                   save_best_only=True, 
    ...
    print ("load_weights:", i)
    featureNet.load_weights(Path['Save']+'FC_MSAN_Best_'+str(i)+'.h5')
    ...
    saveFile = open(Path['Save'] + "Result_FC_MSAN.txt", 'a+')
